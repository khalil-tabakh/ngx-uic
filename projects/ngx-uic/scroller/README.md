...
The `NgxScrollerModule` provides a reusable scrollable container component for Angular applications. It includes the `NgxScrollerComponent`, a customizable Angular component for creating bi-directional infinite scroll containers.
...
⚡ Virtualization for large dynamically sized elements

🔍 IntersectionObserver-driven item visibility tracking

♻️ Windowing with automatic expansion/shrinking

↕️ Emits events when reaching the start or end of the list

📦 Supports horizontal and vertival flexbox, grid, and normal flow

🧭 Works with custom scroll containers & root margins

🔧 Controlled or automatic emission logic
...
```ts
import { NgxScrollerComponent } from 'ngx-scroller';

@Component({
    selector: 'app-demo',
    imports: [NgxScrollerComponent], // <- Add here
...
```html
<ngx-scroller [items]="items()" (last)="onReachedBottom()" #scroller>
...
```html
<ngx-scroller overflow [items]="items()" (first)="onReachedTop()" (last)="onReachedBottom()" #scroller>
    @for (item of scroller.content(); track item) {
        <div>{{ item }}</div>
    }
</ngx-scroller>
...
```html
<ngx-scroller virtualize [items]="items()" (last)="onReachedBottom()" #scroller>
    @for (item of scroller.content(); track item) {
        <div>{{ item }}</div>
    }
</ngx-scroller>
...
```html
<div class="root" #root>
    <ngx-scroller [batch]="5" [container]="container" [items]="items()" [offset]="2" [root]="root" [rootMargin]="100px" (last)="onReachedBottom()" #scroller>
        <div class="container" #container>
            @for (item of scroller.content(); track item) {
                <div>{{ item }}</div>
            }
        <div>
        <button (click)="onReachedBottom()">Reload</button>
    </ngx-scroller>
<div>
```
...
```html
<ngx-scroller [emit]="!loading()" [items]="items()" (last)="onReachedBottom()" #scroller>
    @for (item of scroller.content(); track item) {
        <div>{{ item }}</div>
    }
</ngx-scroller>
...
```html
<ngx-scroller class="grid" [items]="items()" (last)="onReachedBottom()" #scroller>
    @for (item of scroller.content(); track item) {
        <div>{{ item }}</div>
    }
</ngx-scroller>
